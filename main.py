...
# This bit of code creates an avg_rating column and writes it to a new csv
avg_rating = []
...
plt.xlabel("Ratings")
plt.ylabel("Average Amt. Book Moves at Opening")
plt.show()
...
for index in range(len(df["avg_rating"])):
    try: 
        opening_ply_sums[df["avg_rating"][index]] += 1
    except:
        opening_ply_sums[df["avg_rating"][index]] = 0

for item in opening_ply_sums:



avg_rating_avg_opening_ply = {}
 """
...
""" while continue_loop:
    try:
        for index in len(df["white_rating"]):
            avg = ( df["white_rating"][index] + df["black_rating"][index] ) / 2
            avg_rating[index] = avg
        df["avg_rating"] = avg_rating 
        continue_loop = False
    except:
        avg_rating = [] """
